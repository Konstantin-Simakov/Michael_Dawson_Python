...
def main():
	...
	file_name = "exercise3.dat"
	scores_file = shelve.open(file_name)
	
	# Output of the invormation about a previous winner 
	if "winner" in scores_file:
		winner = scores_file["winner"]
		if winner:
			name, score = winner
	...
	# Writing the data to the shelve file.
	winner = []
	winner.append(name)
	winner.append(score)
	scores_file["winner"] = winner
	# Optional sync() method (in this case).
	scores_file.sync()
	scores_file.close()
